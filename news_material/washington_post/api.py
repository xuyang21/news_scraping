...
class ParseResult:
    def __init__(self, url, title, content, pictures, captions):
        self.url = url
        self.title = title
...
def download_image(file_path, url):
    folder_path = 'news_material/washington_post/generate/' + proj_utils.get_current_date().strftime("%Y-%m-%d")
...
# def write_obj_to_file(file_path, obj):
...
def scrap_homepage_titles():
    titles = []
    try:
        response = scrap_url(site_url, site_params,
                             site_cookie, site_headers)
        soup = BeautifulSoup(response.content, 'html.parser')
        tags = soup.find_all('a', {'data-pb-local-content-field': 'web_headline'})
        for i, ele in enumerate(tags):
            href = ele.get('href')
            title = ele.text.strip()
            titles.append(TitleInfo(title, href))
            # print("URL:", href)
            # print("Text:", title)
            # print("---")
...
def get_single_news(url, title):
    try:
        content = []
        response = scrap_url(url, site_params,
                             site_cookie, site_headers)
        soup = BeautifulSoup(response.content, 'html.parser')
        write_to_file("/detail.html", response.text)
        ...
        print("Get washington news failed, url:", url, "title:", title, str(e))


def get_single_news_v2(url, title):
    try:
        response = scrap_url(url, site_params,
                             site_cookie, site_headers)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        pictures = []
        captions = []
        potential = soup.find('script', {'id': '__NEXT_DATA__', 'type': 'application/json'})
        json_data = potential.string
        parsed_data = ujson.loads(json_data)
        field_value = parsed_data['props']["pageProps"]["globalContent"]["content_elements"]
        for element in field_value:
            # print(element)
            additional_properties = element.get('additional_properties')
            if additional_properties:
                # this is a picture
                url = str(element.get('url'))
                if url != "None":
                    # print(url)
                    caption = str(element.get('credits_caption_display'))
        ...
        return " ".join(content_list), pictures, captions

    except Exception as e:
        print("Get washington news failed, url:", url, "title:", title, str(e))


def split_paragraph(paragraph, line_length):
    words = paragraph.split()  # 将段落拆分为单词列表
    lines = []
    current_line = ""

    for word in words:
        if len(current_line) + len(word) + 1 <= line_length:  # 检查是否可以将单词添加到当前行
            if current_line:
                current_line += " "
            current_line += word
        else:
            lines.append(current_line)  # 当前行已满，添加到行列表中
            current_line = word
...
def get_headline_news():
    titles = scrap_homepage_titles()
    # write_to_file("/headLines", "\n".join(str(titles)))
    if len(titles) == 0:
        print("Get empty title from Washington Post")
        return "", ""
    # content = get_single_news(titles[0].url, titles[0].title)
    content, pictures, captions = get_single_news_v2(titles[0].url, titles[0].title)
    # matches = re.findall(r'[@#$%<>]', content)
    # 打印匹配到的特殊字符
    # print(matches)
    content = re.sub(r"<[a-z].*?</[a-z]>", " ", content)
    content = split_paragraph(content, 100)
    result = ParseResult(titles[0].url, titles[0].title, content, pictures, captions)
    write_to_file("/result.txt", str(result))
    idx = 0
    for url in pictures:
        # download_image("/" + str(idx) + "_" + captions[idx] + ".jpg", url)
        download_image("/" + str(idx) + ".jpg", url)
        idx += 1
    return result
